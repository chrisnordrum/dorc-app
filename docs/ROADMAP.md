...
- [x] 0.1 Inspect the Atlas cluster; decide migrate-vs-wipe _(wipe — see findings above)_
...
- [x] 0.2a Rewrite `server/.env.example` so no value in it looks like a usable secret
      _(also fixed a real bug: `node --env-file` doesn't support trailing inline comments,
      so the old `EMAIL_ENCRYPTION_SECRET=mysecretkey12345 //16-byte key` produced a
      30-byte key and an AES failure for anyone following it)_
- [x] 0.2b **Verify Render's environment variables** are the strong secrets, not the old
      placeholders. Rotate anything weak. _(Needs Render dashboard access.)_
...
- [x] 1.1 npm workspaces (`client`, `server`, `packages/shared`), one lockfile. Delete the
  ...
  _(Done. `react-router-dom` moved to `client` dependencies. `tailwindcss` was left in
  ...
  the root lockfile. That is only the lockfile-path fix — 1.6 and 1.8 still rewrite them.)_
...
type` for types).
  ...
  _should_ enforce: they add a username format and length caps that only the client
...
- [x] 1.3 `tsconfig.base.json` with `strict: true` and `allowJs: true`; per-workspace configs
  ...
  *(Done. `checkJs` is deliberately **off**: `allowJs` pulls the existing `.js`/`.jsx` in so
  ...
  (`vite.config`, `postcss.config`, `tailwind.config`) separately, so
...
- [x] 1.4 One flat ESLint config at root covering all workspaces (hoist the existing
      `client/eslint.config.js`), plus Prettier and a `format:check` script.

  *(Done. `eslint.config.mjs` at the root — `.mjs` because the root `package.json` is
  CommonJS. `npm run lint` lints everything; the client's own `lint` script and its ESLint
  devDependencies are gone, and `eslint.config.*` is out of `client/tsconfig.node.json`.
  Blocks by area: React hooks + refresh rules for `client/src`, browser globals there,
  node globals for config files, CommonJS + node globals for `server`, and
  `typescript-eslint` recommended for `.ts` (so `packages/shared` is linted for the first
  time). `eslint-config-prettier` goes last so the two never disagree. Unused arguments
  prefixed `_` are allowed.

  Prettier runs on defaults (`.prettierrc.json` is `{}`) because the code was already
  double quotes, semicolons and trailing commas. `npm run format` writes,
  `npm run format:check` is what 1.6 calls. The whole repo was reformatted in this commit — 30-odd
  files, whitespace only. `.prettierignore` skips the lockfile, `data.json` (the server
  rewrites it at runtime) and `docs/security-coursework.md` (archived as submitted; its
  unfenced ASCII diagrams don't survive reformatting).
...
- [ ] 2.3 `express-rate-limit` — strict on `/api/auth/login`, `/register`, `/refresh`; looser
      ...
      one IP. _(There is currently no rate limiting anywhere.)_
...
- [ ] 2.5 Fix the email encryption: add a deterministic unique-indexed `emailHash`
      ...
      _(Alternative if the custom crypto stops earning its keep: drop field encryption and
      rely on Atlas encryption at rest.)_
...
- [ ] 3.3 **Ownership enforcement on every quest route** — resolve the document, assert
      ...
      serious vulnerability in this kind of app, and these routes currently have _no auth
      middleware at all_. Ship the test (user A cannot read, complete or delete user B's
